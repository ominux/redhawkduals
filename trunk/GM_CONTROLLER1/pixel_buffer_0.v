...
// Input Parameters
parameter DEFAULT_BUFFER_ADDRESS	= 32'h01090000;
parameter DEFAULT_BACK_BUF_ADDRESS	= 32'h01090000;

parameter WIW						= 8;  // Image In Width - Address Width
parameter HIW						= 7;  // Image In Height - Address Width

parameter DW						= 15; // Image In - Data Width

parameter PIXELS_IN					= 320;
parameter LINES_IN 					= 240;

// Output Parameters
parameter WOW						= 9;  // Image Out Width - Address Width
parameter HOW						= 8;  // Image Out Height - Address Width

parameter ENLARGE_WIDTH				= 1;  // Enlarge Pixel Width Amount
parameter ENLARGE_HEIGHT			= 1;  // Enlarge Pixel Height Amount

parameter EH_FIFO_ADDRESS_WIDTH		= 9;
parameter EH_FIFO_SIZE				= 512;
...
// Enlarge height fifo signals
wire		[DW: 0]	eh_fifo_data_in;
wire				eh_fifo_read;
wire				eh_fifo_write;

wire		[DW: 0]	eh_fifo_data_out;
...
//  Input Counters
reg			[WIW:0]	w_address_in;
reg			[HIW:0]	h_address_in;
...
begin
	...
	begin
		...
		if (slave_byteenable[3])
			...
			((w_address_in == (PIXELS_IN - 1)) & 
			(h_address_in == (LINES_IN - 1))))
	begin
...
begin
	if (reset)
		...
		buffer_swap <= 1'b1;
	else if ((w_address_in == 'h0) & (h_address_in == 'h0))
		buffer_swap <= 1'b0;
end
...
begin
	...
	begin
		if (w_address_in == (PIXELS_IN - 1))
			w_address_in <= 'h0;
		else
			w_address_in <= w_address_in + 'h1;
	end
end
...
begin
	if (reset)
		h_address_in <= 'h0;
			...
			(w_address_in == (PIXELS_IN - 1)))
	begin
		if (h_address_in == (LINES_IN - 1))
			h_address_in <= 'h0;
		else
			h_address_in <= h_address_in + 'h1;
	end
end
...
assign master_address		= buffer_start_address + 
								{h_address_in, w_address_in, 1'b0};
...
assign stream_valid			= 
	~db_fifo_empty | (|(h_address_out[(ENLARGE_HEIGHT - 1):0]));

// Internal Assignments
assign red		= (|(h_address_out[(ENLARGE_HEIGHT - 1):0])) ? 
					{2{eh_fifo_data_out[15:11]}} : 
					{2{db_fifo_data_out[15:11]}};
assign green	= (|(h_address_out[(ENLARGE_HEIGHT - 1):0])) ? 
					{eh_fifo_data_out[10: 5], eh_fifo_data_out[10: 7]} : 
					{db_fifo_data_out[10: 5], db_fifo_data_out[10: 7]};
assign blue		= (|(h_address_out[(ENLARGE_HEIGHT - 1):0])) ? 
					{2{eh_fifo_data_out[ 4: 0]}} : 
					{2{db_fifo_data_out[ 4: 0]}};
...
assign db_fifo_read		= 
	(&(w_address_out[(ENLARGE_WIDTH - 1):0])) &
	~(|(h_address_out[(ENLARGE_HEIGHT - 1):0])) &
	stream_ready & stream_valid;

// Enlarge Height fifo assignments
assign eh_fifo_data_in	= (|(h_address_out[(ENLARGE_HEIGHT - 1):0])) ?
							eh_fifo_data_out : 
							db_fifo_data_out;
assign eh_fifo_write	= (&(h_address_out[(ENLARGE_HEIGHT - 1):0])) ? 1'b0 :
							(|(h_address_out[(ENLARGE_HEIGHT - 1):0])) ?
								eh_fifo_read :
								db_fifo_read;

assign eh_fifo_read		= stream_ready & stream_valid & 
							(&(w_address_out[(ENLARGE_WIDTH - 1):0])) &
							(|(h_address_out[(ENLARGE_HEIGHT - 1):0]));
...
scfifo Enlarge_Height_FIFO (
	// Inputs
	.clock			(clk),
	.sclr			(reset),
   
	.data			(eh_fifo_data_in),
	.wrreq			(eh_fifo_write),
...
defparam
	Enlarge_Height_FIFO.add_ram_output_register	= "OFF",
	Enlarge_Height_FIFO.intended_device_family	= "Cyclone II",
	Enlarge_Height_FIFO.lpm_numwords			= EH_FIFO_SIZE,
	Enlarge_Height_FIFO.lpm_showahead			= "ON",
	Enlarge_Height_FIFO.lpm_type				= "scfifo",
	Enlarge_Height_FIFO.lpm_width				= DW + 1,
	Enlarge_Height_FIFO.lpm_widthu				= EH_FIFO_ADDRESS_WIDTH,
	Enlarge_Height_FIFO.overflow_checking		= "OFF",
	Enlarge_Height_FIFO.underflow_checking		= "OFF",
	Enlarge_Height_FIFO.use_eab					= "ON";
